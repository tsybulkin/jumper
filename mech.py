...
def Lf(x,y,a,b):
	g,m1,m2,L1,L2,L3,I1,I2,eps,gama0,z0,z1,z2,z3,k1,k2,k3,dz,da,psi = \
	symbols('g m1 m2 L1 L2 L3 I1 I2 eps gama0 z0 z1 z2 z3 k1 k2 k3 dz da psi')

	x_d, y_d, a_d, b_d, gama_d = dynamicsymbols('x y a b',1)

	x1 = x + L3*cos(b) + L2*cos(b+gama0) + L1*cos(a+b+gama0)
	y1 = y + L3*sin(b) + L2*sin(b+gama0) + L1*cos(a+b+gama0)

	x2 = x + L3*cos(b) + L2*eps*cos(b+gama0)
	y2 = y + L3*sin(b) + L2*eps*sin(b+gama0)
	...
	V = m1*g*y1 + k1/2*u1**2 + k2/2*u2**2 + m2*g*y2
