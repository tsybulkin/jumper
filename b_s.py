...
def get_c2(q, q_d,psi=0):
	...
	return np.array([
		...
		+ 2*L1*L3*m1*sin(a + b + g)*sin(b) \
		- 2*L1*L3*m1*sin(a + b + g)*cos(b) \
		+ 2*L2*L3*eps*m2*sin(b + g)*sin(b) \
		+ 2*L2*L3*eps*m2*cos(b + g)*cos(b) \
		+ 2*L2*L3*m1*sin(b + g)*sin(b) \
		+ 2*L2*L3*m1*cos(b + g)*cos(b), 
...
def get_d2(q, q_d, psi=0):
	...
	return L1**2*m1*sin(2*a + 2*b + 2*g)*a_d**2 \
		...
		+ L1*L3*m1*sin(b)*cos(a + b + g)*a_d**2 \
		+ 2*L1*L3*m1*sin(b)*cos(a + b + g)*a_d*b_d \
		+ 2*L1*L3*m1*sin(b)*cos(a + b + g)*a_d*g_d \
		+ L1*L3*m1*sin(b)*cos(a + b + g)*b_d**2 \
		+ 2*L1*L3*m1*sin(b)*cos(a + b + g)*b_d*g_d \
		+ L1*L3*m1*sin(b)*cos(a + b + g)*g_d**2 \
		- L1*L3*m1*cos(a + b + g)*cos(b)*a_d**2 \
		- 2*L1*L3*m1*cos(a + b + g)*cos(b)*a_d*b_d \
		- 2*L1*L3*m1*cos(a + b + g)*cos(b)*a_d*g_d \
		- L1*L3*m1*cos(a + b + g)*cos(b)*b_d**2 \
		- 2*L1*L3*m1*cos(a + b + g)*cos(b)*b_d*g_d \
		- L1*L3*m1*cos(a + b + g)*cos(b)*g_d**2 \
		- L1*Grav*m1*sin(a + b + g) \
		- 2*L2*L3*eps*m2*sin(b + g)*cos(b)*b_d*g_d \
		- L2*L3*eps*m2*sin(b + g)*cos(b)*g_d**2 \
		+ 2*L2*L3*eps*m2*sin(b)*cos(b + g)*b_d*g_d \
		+ L2*L3*eps*m2*sin(b)*cos(b + g)*g_d**2 \
		- 2*L2*L3*m1*sin(b + g)*cos(b)*b_d*g_d \
		- L2*L3*m1*sin(b + g)*cos(b)*g_d**2 \
		+ 2*L2*L3*m1*sin(b)*cos(b + g)*b_d*g_d \
		+ L2*L3*m1*sin(b)*cos(b + g)*g_d**2 \
		+ L2*eps*Grav*m2*cos(b + g) + L2*Grav*m1*cos(b + g) \
		+ L3*Grav*m1*cos(b) + L3*Grav*m2*cos(b)
